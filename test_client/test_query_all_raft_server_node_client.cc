 ...
 *    #			###   #		  #				#				 #   #		#					####
 *
 * test_query_all_raft_server_node_client.cc
...
void test_client() {
  ...
  ::QueryAllRaftServerNodeRequest rpc_req;
  ::QueryAllRaftServerNodeResponse rpc_res;

  std::cout << "Send to tinyrpc server " << addr->toString() << ", requeset body: " << rpc_req.ShortDebugString() << std::endl;
  stub.QueryAllRaftServerNode(&rpc_controller, &rpc_req, &rpc_res, NULL);
